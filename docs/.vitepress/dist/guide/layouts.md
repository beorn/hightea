...
CSS gives you `fit-content` (widest wrapped line) and greedy word-wrap. There's no way to say *"find the narrowest width that still produces exactly 3 lines"* or *"break lines to minimize raggedness across the whole paragraph."*
...
`snug-content` finds the *narrowest* width that still produces the same number of lines. Pretext calls this ["shrinkwrap"](https://chenglou.me/pretext/bubbles/) — it uses [`walkLineRanges()`](https://github.com/chenglou/pretext) to binary-search over widths, comparing line counts at each candidate. Silvery uses the same algorithm adapted for terminal character-cell grids:
