...
*Colors, typography, and component defaults for the shiniest Silvery apps*
...
`$accent` is the **complement** of `$primary` — a contrasting hue for momentary attention. In truecolor themes, it's the color wheel opposite of primary. Use it when something needs to pop *against* the brand color without implying status.
