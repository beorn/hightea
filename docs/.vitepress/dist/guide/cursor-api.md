...
The key insight is that `useBoxRect()` provides *absolute* terminal coordinates via `x` and `y`. This allows cursor positioning relative to a component:
