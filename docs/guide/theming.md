...
The `maybeDarkBackground` field on `createTerminalProfile().caps` still uses synchronous environment-variable heuristics and the macOS `defaults` command. The `maybe` prefix is deliberate — it's a guess, not a protocol-verified fact. The Mode 2031 detector is async and designed for apps that can wait for terminal responses at startup for a definitive answer.
