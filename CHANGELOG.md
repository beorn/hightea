...
- Synchronized Update Mode (DEC 2026): all TTY output is wrapped with `CSI ? 2026 h` / `CSI ? 2026 l` for flicker-free rendering. Terminals paint atomically instead of showing intermediate states. Disable with `HIGHTEA_SYNC_UPDATE=0`.
...
- **Inspector**: `enableInspector()` / `HIGHTEA_DEV=1` for render stats, tree dumps, and dirty flag visualization
...
- **TEA sub-path barrel exports**: `hightea/core`, `hightea/store`, `hightea/react` for tree-shakeable imports
...
- Pluggable layout engines: Flexture (default) and Yoga (WASM)
