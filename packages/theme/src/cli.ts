...
function showTheme(name: string): void {
  ...
  console.log(`  ${bg(previewBg, fg(theme.primary.startsWith("#") ? theme.primary : palette.blue, `  Primary text          `))}`);
  console.log(`  ${bg(previewBg, fg(theme.success.startsWith("#") ? theme.success : palette.green, `  Success message       `))}`);
  console.log(`  ${bg(previewBg, fg(theme.error.startsWith("#") ? theme.error : palette.red, `  Error message         `))}`);
  console.log();
}
...
async function interactiveBrowser(): Promise<void> {
  ...
  function render(): void {
    ...
    // Header
    process.stdout.write(`  \x1b[1m@silvery/theme\x1b[0m — ${names.length} palettes  (j/k navigate, Enter show, q quit)\n\n`);
