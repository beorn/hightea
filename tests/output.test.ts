...
describe('Output Functions', () => {
	describe('styleToAnsi', () => {
		...
		test('converts blink attribute', () => {
			const result = styleToAnsi({ fg: null, bg: null, attrs: { blink: true } });
			expect(result).toContain('5'); // blink SGR code
		});

		test('converts hidden attribute', () => {
			const result = styleToAnsi({ fg: null, bg: null, attrs: { hidden: true } });
			...
			const changes = [
				{
					x: 0,
					y: 0,
					cell: { char: 'X', fg: null, bg: null, attrs: {}, wide: false, continuation: false },
				},
			];
			...
			const changes = [
				{
					x: 0,
					y: 0,
					cell: { char: '\u4e2d', fg: null, bg: null, attrs: {}, wide: true, continuation: false },
				},
				{
					x: 2, // After wide char, cursor should be at 2 (not 1)
					y: 0,
					cell: { char: 'A', fg: null, bg: null, attrs: {}, wide: false, continuation: false },
				},
			];
			const result = changesToAnsi(changes);
			...
			const changes = [
				{
					x: 5,
					y: 1,
					cell: { char: 'B', fg: null, bg: null, attrs: {}, wide: false, continuation: false },
				},
				{
					x: 0,
					y: 0,
