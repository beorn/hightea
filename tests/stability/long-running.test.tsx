...
describe("stability: sustained rendering", () => {
  ...
  test(
    "sustained rendering with complex layout",
    async () => {
  ...
  test("sustained incremental rendering matches fresh", async () => {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      if (i % 50 === 0) {
        ...
        // Compare text output
        const { bufferToText } = await import("@silvery/test");
...
describe("stability: resize handling", () => {
  ...
  test("resize with useContentRect updates correctly", () => {
    ...
    // Wide — should show "Wide layout"
    expect(app.text).toContain("Wide layout");

    // Shrink — should switch to "Narrow layout"
    app.resize(30, 24);
    expect(app.text).toContain("Narrow layout");

    // Expand again — should switch back
    app.resize(80, 24);
    expect(app.text).toContain("Wide layout");
...
describe("stability: mixed operations", () => {
  ...
  test("unmount during active input does not crash", async () => {
    ...
    expect(() => app.press("j")).rejects.toThrow();
  });
